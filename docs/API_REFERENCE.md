...
- 路径参数：`source_id` 当前受管授权只支持 `cecbid`。千里马使用公开分类列表，并提供“免费登录并在原站查询”的用户前台入口；不会保存或后台重放会员 Cookie，也不会把该入口接入定时批量抓取。
...
- 用途：携带已加密保存的会话依次执行少量高可用探针词的有界真实搜索，验证会话同时作用于搜索与详情，并且确实读取到免费会员可见内容，而不是只检查“数据库里有 Cookie”。
- 路径参数：`source_id`；必须携带短期编辑令牌；远端请求还要符合当前 LAN 策略；无请求体。
- 返回：`source_id`、`success`、`passed/failed/inconclusive`、脱敏诊断和耗时。零结果会返回 `inconclusive` 而不是误报登录失败；只有 `passed` 会进入后台检索，且每次通过后的最长信任期为 7 天。
