...
局域网设备可以点击来源中心的“打开浏览器授权”“完成授权”“测试授权”和“清除”。当前 Cookie 型授权的可见 Chromium 会在运行 BidPilot 服务的电脑上弹出，而不是在手机上弹出；你需要在服务电脑亲自登录，然后回到任意管理页面点击完成。完成后会自动验证真实搜索与免费会员详情，只有验证通过的会话才用于后台检索；捕获未验证、无法判定、失败或过期都不会显示为已增强。只有平台官方提供 OAuth、扫码确认或设备码时，远端设备才可能直接完成。
