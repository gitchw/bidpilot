...
from bidpilot.models import HealthResponse, RunResult, SubscriptionCreate, TenderQuerySpec
from bidpilot.scheduler import SubscriptionScheduler
...
def create_app(
    ...
    async def lifespan(_: FastAPI):
        scheduler = SubscriptionScheduler(service)
    ...
    async def create_subscription(request: SubscriptionCreate):
        try:
            return service.create_subscription(
                request.name, request.query, request.delivery_channel
            )
        except ValueError as exc:
            raise HTTPException(status_code=422, detail=str(exc)) from exc
